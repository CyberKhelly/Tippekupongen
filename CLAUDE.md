...
- **NT-only validation mode (2026-07-01):** `_market_scan_job()` in `backend/scheduler.py` now calls `generate_global_bet_candidates(nt_only=True)`. Only bets where `bookmaker = "NT Oddsen"` are generated. AF fallback (Bet365/Marathonbet/William Hill) is disabled for bet generation. 25 AF bets from the 17:15 scheduler run were deleted. Scheduler log now prints `NT-matched: 1x2=N btts=N ou=N | AF-fallback: 0`. Objective: accumulate clean NT Oddsen bet history to validate model performance. Re-enable by reverting to `nt_only=False` in `_market_scan_job()`.
...
- **NT-only Modellspill mode + clean launch (2026-06-30):** `generate_global_bet_candidates(nt_only=True)` skips BTTS and O/U when no NT Oddsen match found — no AF/Bet365 fallback for user-facing bets. `delete_all_model_bets()` in `db/paper_bets.py` hard-deletes all rows (used for ledger resets). `scripts/reset_and_rescan.py` performs the full reset sequence: delete all bets → scrape NT → enrich NT fixtures → generate NT-only candidates → verify. All model bets were reset after NT Oddsen ingestion, true EV, and placeholder-odds guards were completed. **Validation phase (2026-07-01): the scheduler's `_market_scan_job()` now uses `nt_only=True`. Only NT Oddsen bets are generated. 25 AF fallback bets (Bet365/Marathonbet/William Hill) were deleted on 2026-07-01. Re-enable `nt_only=False` after enough NT history exists to evaluate model performance.**
- **NT Oddsen fixture enrichment (2026-06-30):** `enrich_nt_oddsen_fixtures()` in `ingestion/enrich_fixtures.py` enriches any NT Oddsen fixture that lacks a `fixture_stat_enrichment` row, regardless of whether it appears in the NT Tipping coupon. For fixtures not yet in the DB (outside the 72h scan window — e.g. Canada vs Morocco, Colombia vs Ghana on 2026-07-04), `_fetch_and_insert_wc_fixture()` fetches them from AF WC league, inserts them into `fixtures` + `api_football_fixture_links`, fetches bookmaker odds, and then enriches. Called between NT scrape and bet generation in `reset_and_rescan.py`. Result: coverage went from 3/13 full_model to 13/13 enriched (11 newly enriched + 2 new fixtures created). Team name normalization additions: `"cape verde islands"` → `"cape verde"`, `"sterrike"` → `"austria"` (NT "Østerrike" loses Ø → "sterrike").
